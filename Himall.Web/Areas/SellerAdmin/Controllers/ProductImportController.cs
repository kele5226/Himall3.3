...
namespace Himall.Web.Areas.SellerAdmin.Controllers
{
    public class ProductImportController : BaseSellerController
    {
                ...
                try
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        if (item != null && item.ProductInfo != null)
                        {
                            ...
                            {
                                createModel.Quantity = Himall.Core.Helper.TypeHelper.StringToInt(item.ProductInfo.NoSkuInfo.data.dynStock.stock);
                            }
                            ...
                            if (item.ProductInfo.propertyPics != null)
                            {
                                ...
                                for (int i = 0; i < imagesPics.Count; i++)
                                {
                                    ...
                                    imagesPics[i] = Core.Helper.HtmlContentHelper.TransferNetworkImageToLocal(imagesPics[i], "/", imageRealtivePath, Core.HimallIO.GetImagePath(imageRealtivePath) + "/");
                                }
                            ...
                            if (item.ProductInfo.api != null)//商品详情内容
                            {
                                ...
                                createModel.Description.Description = Core.Helper.HtmlContentHelper.TransferToLocalImage(createModel.Description.Description, "/", imageRealtivePath, Core.HimallIO.GetImagePath(imageRealtivePath) + "/", true);//处理详情图片
                            ...
                            {
                                List<SpecificationValueInfo> specifications = ObjectContainer.Current.Resolve<ISpecificationService>().GetSpecification(model.SmallCategoryId, this.CurrentShop.Id);
                                   ...
                                   if (colorItem != null)
                                    {
                                        ...
                                        for (int j = 0; j < skuPropertyValues.Count; j++)
                                        {
                                            ...
                                            if (skuPropertyValues[j].PropertyValueIcon != "htt")
                                                skuPropertyValues[j].PropertyValueIcon = Core.Helper.HtmlContentHelper.TransferNetworkImageToLocal(skuPropertyValues[j].PropertyValueIcon, "/", imageRealtivePath, Core.HimallIO.GetImagePath(imageRealtivePath) + "/");//处理规格图片
                                        }
                        ...
                        //2、淘宝商品本身有规格，Himall平台的类型没勾选规格，则取规格中库存最大的项
                        ...
                        {
                            createModel.Quantity = item.ProductInfo.valItemInfo.skuMap.Select(a => Himall.Core.Helper.TypeHelper.StringToInt(a.stock)).Max();
                        }
        ...
        public JsonResult GetPlatFormCategory(long? key = null, int? level = -1)
        {
            ...
            if (key.HasValue)
            {
                var shopcategories = ObjectContainer.Current.Resolve<IShopCategoryService>().GetBusinessCategory(CurrentSellerManager.ShopId, this.CurrentShop.IsSelf).Select(e => e.Id);
