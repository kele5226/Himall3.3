            ...
            var ret = DbFactory.Default.InTransaction(() =>
            {
                if (refund.RefundPayStatus == OrderRefundInfo.OrderRefundPayStatus.PaySuccess)
                {
                    ...
                    */
                    ...
                    if (refund.RefundMode == OrderRefundInfo.OrderRefundMode.OrderRefund)
                    {
                        Himall.ServiceProvider.Instance<IDistributionService>.Create.TreatedOrderDistributionBrokerage(refund.OrderId, false);
                    }
                    else
                    {
                        Himall.ServiceProvider.Instance<IDistributionService>.Create.TreatedOrderDistributionBrokerage(refund.OrderId, false, refund.OrderItemId);
                    }
                    #endregion
