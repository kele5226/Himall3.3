    ...
    public class MemberService : ServiceBase, IMemberService
    {
            ...
            var flag = DbFactory.Default.InTransaction(() =>
            {
                ...
                if(!string.IsNullOrEmpty(mobile) || !string.IsNullOrEmpty(email))
                {
                    ...
                    if (!string.IsNullOrEmpty(mobile) && Core.Helper.ValidateHelper.IsMobile(mobile)) //绑定手机号
                    {
                        ...
                        //info.TypeId = Himall.Entities.MemberIntegralInfo.IntegralType.Reg;
                        info.ReMark = "绑定手机";
                        //var memberIntegral = ServiceProvider.Instance<IMemberIntegralConversionFactoryService>.Create.Create(Himall.Entities.MemberIntegralInfo.IntegralType.Reg);
        ...
        public void AddIntegel(MemberInfo member)
        {
            if (!ServiceProvider.Instance<IMemberIntegralService>.Create.HasLoginIntegralRecord(member.Id)) //当天没有登录过，加积分
            {
                ...
                var memberIntegral = ServiceProvider.Instance<IMemberIntegralConversionFactoryService>.Create.Create(Himall.Entities.MemberIntegralInfo.IntegralType.Login);
                ServiceProvider.Instance<IMemberIntegralService>.Create.AddMemberIntegral(info, memberIntegral);
            }
        }
        ...
        private void AddBindInergral(MemberInfo member)
        {
            ...
            try
            {
                ...
                info.ReMark = "绑定微信";
                var memberIntegral = ServiceProvider.Instance<IMemberIntegralConversionFactoryService>.Create.Create(Himall.Entities.MemberIntegralInfo.IntegralType.BindWX);
                ServiceProvider.Instance<IMemberIntegralService>.Create.AddMemberIntegral(info, memberIntegral);
            }
            catch (Exception ex)
            {
