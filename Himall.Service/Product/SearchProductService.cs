...
namespace Himall.Service
{
    public class SearchProductService : ServiceBase, ISearchProductService
    {
        ...
        {
            var sql = new Sql();
            sql.Append("INSERT INTO `himall_searchproduct` (`ProductId`, `ProductName`, `ShopId`, `ShopName`, `BrandId`, `BrandName`, `FirstCateId`, `FirstCateName`, `SecondCateId`, `SecondCateName`, `ThirdCateId`, `ThirdCateName`, `AttrValues`, `Comments`, `SaleCount`, `SalePrice`, `OnSaleTime`, `ImagePath`, `CanSearch`, `BrandLogo`) ");
            sql.Append("select a.Id,a.ProductName,a.ShopId,b.ShopName,a.BrandId,c.`Name` as BrandName,SUBSTRING_INDEX(a.CategoryPath, '|', 1) AS FirstCateId, d.Name as FirstCateName,SUBSTRING_INDEX(SUBSTRING_INDEX(CategoryPath, '|', 2), '|', -1) AS SecondCateId, e.Name as SecondCateName,SUBSTRING_INDEX(CategoryPath, '|', -1) AS ThirdCateId, f.Name as ThirdCateName,g.AttrValues,0,0,a.MinSalePrice,a.AddedDate,a.ImagePath,(case when a.SaleStatus=1 and a.AuditStatus=2 and a.IsDeleted=0 then 1 else 0 end) as CanSearch,c.Logo from himall_product a ");
        ...
        public SearchProductFilterResult SearchProductFilter(SearchProductQuery query)
        {
            try
            {
                ...
                var allCategorys = ObjectContainer.Current.Resolve<ICategoryService>().GetCategories();
