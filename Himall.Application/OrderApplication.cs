    ...
    public class OrderApplication : BaseApplicaion<IOrderService>
    {
        ...
        public static OrderReturnModel SubmitOrder(CommonModel.OrderPostModel submitModel, string payPwd = "")
        {
            if (submitModel.Capital > 0 && !string.IsNullOrEmpty(payPwd))
            {
                ...
                if (!flag)
                {
                    throw new HimallException("预存款支付密码错误");
                }
            }
            ...
            {
                throw new HimallException("预存款金额不正确");
            }
            ...
            foreach (var order in models)
            {
                ...
                foreach (var item in order.OrderItems)
                {
                    ...
                    if (p != null)
                    {
                        ...
                        item.ThumbnailsUrl = HimallIO.GetRomoteProductSizeImage(item.ThumbnailsUrl, 1, (int)Himall.CommonModel.ImageSize.Size_100);
                    }
        ...
        /// </summary>
        /// <returns></returns>
        static Himall.Entities.ShoppingCartInfo GetCart(long memberId, string cartInfo)
        {
