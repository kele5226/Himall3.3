...
namespace Himall.SmallProgAPI
{
    public class ProductController : BaseApiController
    {
        ...
        public JsonResult<Result<dynamic>> GetProductList(long shopId, long shopBranchId, string keyWords = "", long? productId = null, long? shopCategoryId = null, long? categoryId = null, int type = 0, int pageNo = 1, int pageSize = 10)
        {
            ...
            var product = pageModel.Models.ToList().Select(item =>
            {
                ...
                return new
                {
                    ...
                    DefaultImage = HimallIO.GetRomoteProductSizeImage(item.RelativePath, 1, (int)ImageSize.Size_350),
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public JsonResult<Result<dynamic>> GetProductDetail(string openId, long ProductID, long shopBranchId = 0)
        {
            //CheckUserLogin();
            if (shopBranchId > 0)
            {
            ...
            List<Himall.Entities.ShoppingCartItem> cartitems = CartApplication.GetCartQuantityByIds(userId, Ids);
        ...
        public JsonResult<Result<dynamic>> GetProductSkus(long productId, string openId = "")
        {
            ...
            Himall.Entities.ShoppingCartInfo cartInfo = CartApplication.GetCart(CurrentUser.Id);
        ...
        public JsonResult<Result<dynamic>> GetLoadReview(long productId, int pageIndex, int pageSize, int type, long shopBranchId = 0)
        {
            ...
            var data = comments.Models.Select(item =>
            {
                ...
                string picture = HimallIO.GetRomoteImagePath(members.FirstOrDefault(p => p.Id == item.UserId)?.Photo ?? string.Empty);
                ...
                for (int i = 0; i < Images.Count; i++)
                {
                    if (i == 0)
                    {
                        images1 = HimallIO.GetRomoteImagePath(MoveImages(Images[i]));
                    }
                    if (i == 1)
                    {
                ...
                return new
                {
                    ...
                    ProductId = item.ProductId,
                    ProductName = product.ProductName,
                    ThumbnailUrl100 = Core.HimallIO.GetRomoteImagePath(product.ImagePath),
                    ...
                    Images4 = images4,
                    Images5 = images5,
                    AppendImages = commentImages.Where(a => a.CommentId == item.Id && a.CommentType == 1).Select(a => new { CommentImage = Core.HimallIO.GetRomoteImagePath(MoveImages(a.CommentImage)) }),//追加图片
        ...
        private int Receive(long vshopId, long couponId)
        {
            ...
            if (CurrentUser != null)
            {
                ...
                if (couponInfo.ReceiveType == Himall.Entities.CouponInfo.CouponReceiveType.IntegralExchange)
                {
        ...
        /// <returns></returns>
        public JsonResult<Result<dynamic>> GetShopProductDetail(long id, long shopBranchId)
        {
            CheckOpenStore();
            //var _iProductService = ServiceApplication.Create<IProductService>();
        ...
        private string GetProductDescription(Entities.ProductDescriptionInfo productDescription)
        {
            ...
            string description = productDescription.ShowMobileDescription.Replace("src=\"/Storage/Shop", "src=\"" + Core.HimallIO.GetRomoteImagePath("/Storage/Shop"));//商品描述
            ...
            if (productDescription.DescriptionPrefixId != 0)
            {
                ...
                descriptionPrefix = desc == null ? "" : desc.MobileContent.Replace("src=\"/Storage/Shop", "src=\"" + Core.HimallIO.GetRomoteImagePath("/Storage/Shop"));
            }
            ...
            if (productDescription.DescriptiondSuffixId != 0)
            {
                ...
                descriptiondSuffix = desc == null ? "" : desc.MobileContent.Replace("src=\"/Storage/Shop", "src=\"" + Core.HimallIO.GetRomoteImagePath("/Storage/Shop"));
            }
