...
namespace Himall.SmallProgAPI
{
    ...
    public class OrderController : BaseApiController
    {
        ...
        //        })
        //    };
        //    return JsonResult<dynamic>(new { ShipTo = order.ShipTo, CellPhone = order.CellPhone, Address = order.RegionFullName + order.Address, ShipOrderNumber = order.DeliveryType == DeliveryType.ShopStore ? "" : order.ShipOrderNumber, ExpressCompanyName = order.DeliveryType == DeliveryType.ShopStore ? "门店店员配送" : order.ExpressCompanyName, LogisticsData = json });
        //}
        ...
        public JsonResult<Result<dynamic>> GetSubmitModel(string cartItemIds, string productSku, WXSmallProgFromPageType fromPage, long buyAmount = 0, long shippingAddressId = 0, string couponIds = "", bool isStore = false, sbyte productType = 0, long shopBranchId = 0)
        {
            CheckUserLogin();
        ...
        JsonResult<Result<dynamic>> GetSubmitModelById(string skuId, int count, long shippingAddressId = 0, IEnumerable<string[]> CouponIdsStr = null, bool isStore = false, sbyte productType = 0, long shopBranchId = 0)
        {
            CheckUserLogin();
            ...
            if (isStore)
            {
                ...
                //门店订单
                Himall.DTO.ShopBranch storeInfo = Application.ShopBranchApplication.GetShopBranchById(shopBranchId);
                if (storeInfo == null)
        ...
        /// <returns></returns>
        JsonResult<Result<dynamic>> GetSubmitByCartModel(string cartItemIds = "", long shippingAddressId = 0, IEnumerable<string[]> CouponIdsStr = null, bool isStore = false)
        {
            CheckUserLogin();
        ...
        private JsonResult<Result<dynamic>> SubmitOrderById(OrderSubmitOrderModel value)
        {
            ...
            try
            {
                ...
                #endregion
                return JsonResult<dynamic>(new { OrderId = string.Join(",", orderIds), OrderTotal = orderTotals,
        ...
        //        var parentAreaInfo = RegionApplication.GetRegion(shippingAddressInfo.RegionId, Himall.CommonModel.Region.RegionLevel.Town);//判断当前区域是否为第四级
        ...
        /// <returns></returns>
        public JsonResult<Result<dynamic>> GetGroupOrderModel(string skuId, int count, long GroupActionId, long? GroupId = null)
        {
            CheckUserLogin();
            dynamic d = new System.Dynamic.ExpandoObject();
        ...
        /// <returns></returns>
        public JsonResult<Result<dynamic>> PostSubmitFightGroupOrder(OrderSubmitFightGroupOrderModel value)
        {
            CheckUserLogin();
            if (value.CapitalAmount > 0 && !string.IsNullOrEmpty(value.PayPwd))
            {
            ...
            try
            {
                ...
                return JsonResult<dynamic>(new { OrderIds = ret.OrderIds, RealTotalIsZero = ret.OrderTotal == 0 });
            }
            catch (HimallException he)
            {
                return Json(ErrorResult<dynamic>(he.Message));
